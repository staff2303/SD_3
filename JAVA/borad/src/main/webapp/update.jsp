...
try {
	...
	DB.result.next();
	no = DB.result.getString("b_no");
	title = DB.result.getString("b_title");
	writer = DB.result.getString("b_writer");
	...
	content = DB.result.getString("b_content");
} catch (Exception e) {
	e.printStackTrace();
}
%>
		...
		<div id="main-wrapper">
			<div class="inner">
				<section class="container box feature1">
					<form action="updatepost.jsp?num=<%=no%>" method="post">
						<table style="text-align: center; border: 1px solid #ffffff">
							...
							<tr>
								<td
									style="background-color: #45474c; text-align: center; width: 10%">번호</td>
								<td style="background-color: #45474c; text-align: left;"><%=no%></td>
								<td
							...
							<tr>
								...
								<td style="background-color: #45474c; text-align: left;"><input
									type="text" style="color: #000000; width: 100%" name="title" required value="<%=title%>"></td>
								<td
									style="background-color: #45474c; text-align: center; width: 10%"></td>
							</tr>
							<tr>
								<td style="background-color: #45474c; text-align: center;">작성자</td>
								<td style="background-color: #45474c; text-align: left;"><%=writer%></td>
								<td
									style="background-color: #45474c; text-align: center; width: 10%"></td>
							</tr>
							<tr>
								...
								<td style="background-color: #45474c; text-align: left;"><textarea
										style="color: #000000; resize: none; width: 100%" name="content"
										oninput="adjustTextareaHeight(this)" required><%=content%></textarea></td>
								<td
									style="background-color: #45474c; text-align: center; width: 10%"></td>
							</tr>
							<tr>
								<td colspan="3" style="background-color: #45474c;"><input
									type="submit" value="수정" onclick="return confirm('수정하시겠습니까?')" class="paging-button"></td>
							</tr>
						</table>
					</form>
				</section>
